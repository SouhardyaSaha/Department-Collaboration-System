...
export class RoutineGeneratorComponent implements OnInit {
  ...
  displayedColumns: string[] = ['Position1','Position2','Position3','Position4','Position5','Position6',
  ...
  ngOnInit(): void {
    ...
    this.routineSub = this.routineService.getRoutineUpdate()
  ...
  //Showing in the grid
    ...
    for(let v = 0;v< 5;v++)
    {
      this.tiles.push({text: array[v].text, cols: 1, rows: 1, color: array[v].color});
      let initialcols = 0;
      if(sortedArray[v].length > 0)
      {
        initialcols = this.stringToInt(this.routineList[sortedArray[v][0].id].startTime) - 8;
      }
      else
      {
        initialcols = 9;
      ...
      let circle = 0,preVal = 0;
      for(let item of sortedArray[v])
      {
        if(circle>0)
        {
          let midDis = 0;
          midDis = this.stringToInt(this.routineList[item.id].startTime)-preVal;
          initialcols+=midDis;
        ...
        let textval  = this.routineList[item.id].courseTitle+"\n"+this.routineList[item.id].instructorName+"\n"+this.routineList[item.id].roomNum,
        colsval =  preVal-this.stringToInt(this.routineList[item.id].startTime) ;
        initialcols+=colsval;
        // console.log(textval);
        this.tiles.push({text: textval, cols: colsval, rows: 1, color: array[v].color});
        circle++;
      }
  ...
  onDownloadItem(val:number){
    ...
    if(val == 1)
    {
        this.exportAsService.save(this.exportAsPngConfig, 'ClassRoutine').subscribe(() => {
        // save started
      });
    }
    else
    {
      this.exportAsService.save(this.exportAsPdfConfig, 'ClassRoutine').subscribe(() => {
        // save started
      });
    }
