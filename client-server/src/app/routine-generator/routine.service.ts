...
export class RoutineService{
  ...
  getRoutineData(){
    this.http.get<{msg:string,routines:RoutineData[]}>('http://localhost:3000/routine')
    .subscribe((data)=>{
      console.log(data);
  ...
  updateRoutineDataById(id,routine:RoutineData){
    ...
    this.http.put<{message:string}>("http://localhost:3000/routine/"+id,postRoutine)
  ...
  addRoutineData(routine:RoutineData){
    ...
    this.http.post<{message:string,id:number}>('http://localhost:3000/routine',postRoutine)
    .subscribe((responseData)=>{
  ...
  deleteRoutineData(id:number){
    this.http.delete("http://localhost:3000/routine/"+id)
    .subscribe((responseData)=>{
      console.log(responseData);
      const UpdatedList = this.routineData.filter((routine)=> routine.id !== id);
      this.routineData = UpdatedList;
      this.updateRoutine.next([...this.routineData]);
    })
  }
