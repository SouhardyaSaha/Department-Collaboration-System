...
export function TimeValidator(control: AbstractControl):{[key:string]:boolean}|null{
  ...
  const Time1 = startTime.value,Time2 = endTime.value;
  ...
  let values1 = 0,values2 = 0;
  if(period[1] == "PM")
  {
    period = period[0].split(":");
    if(period[0] != "12")
      values1 = Number(period[0])+12;
    else
  ...
  if(values1 <8 || values1>17 || values2 < 8 || values2 > 17 ||  values1>=values2 )
  {
    return {'match' :true};
  }
  else
